...
import { usePortfolioStore, type Availability, type AvailabilitySlot } from '../../stores/portfolio'
...
const availabilityData = reactive<Availability>({
  ...
  preferredCommunication: portfolioStore.availability.preferredCommunication || 'email'
})
    ...
    <div class="form-container">
      ...
      <div class="form-group">
        ...
        <div class="add-slot">
          ...
          <div v-if="errors.timeRange" class="form-error">{{ errors.timeRange }}</div>
        </div>
        
        <div v-if="errors.slots" class="form-error">{{ errors.slots }}</div>
        
        <div v-if="availabilityData.slots.length > 0" class="availability-slots">
          <div v-for="(slot, index) in availabilityData.slots" :key="index" class="slot-item">
            <div class="slot-details">
              <span class="slot-day">{{ slot.day }}</span>
              <span class="slot-time">{{ formatTime(slot.startTime) }} - {{ formatTime(slot.endTime) }}</span>
            </div>
            <button @click="removeTimeSlot(index)" class="btn-remove" aria-label="Remove time slot">
              ✕
            </button>
          </div>
        </div>
      ...
      <div v-if="availabilityData.slots.length > 0" class="availability-summary">
