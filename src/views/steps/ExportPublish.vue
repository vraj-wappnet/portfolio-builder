...
const generateLink = () => {
  ...
  setTimeout(() => {
    try {
      generatedLink.value = "http://localhost:5173/portfolio-builder/preview";
      console.log("Generated link:", generatedLink.value);
      isGeneratingLink.value = false;
      generateQRCode();
    } catch (error) {
      console.error("Error generating link:", error);
...
const generateQRCode = async () => {
  ...
  try {
    if (!generatedLink.value) {
      qrError.value = "No link available for QR code";
      return;
    }

    qrCodeUrl.value = await QRCode.toDataURL(generatedLink.value, {
      width: 300, // Increased for better scannability
      margin: 2,
      errorCorrectionLevel: "H",
    });
    console.log("QR code generated for:", generatedLink.value);
  } catch (error) {
    console.error("Error generating QR code:", error);
    qrError.value = "Failed to generate QR code";
  } finally {
    ...
    <div class="export-options">
      <div class="export-option">
        ...
        <button
          ...
          :disabled="isGeneratingPdf"
        >
          <span v-if="isGeneratingPdf">Downloading...</span>
          <span v-else>Download PDF</span>
        </button>
      </div>
      ...
      <div class="export-option">
        ...
        <button
          ...
          :disabled="isGeneratingLink || isGeneratingQR"
        >
          <span v-if="isGeneratingLink || isGeneratingQR">Downloading...</span>
          <span v-else>Generate Link & QR</span>
        </button>
